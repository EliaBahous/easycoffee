...
public class orderProductTests {
    ...
    public void orderProduct() throws IOException {
        URL obj = new URL( "http://localhost:8080/adjust-table");
    ...
    public void checkProductAfterOrder() throws IOException {
        URL obj = new URL( "http://localhost:8080/get-ordered-items");
        HttpURLConnection httpURLConnection = (HttpURLConnection) obj.openConnection();
        httpURLConnection.setRequestMethod("POST");
        httpURLConnection.setRequestProperty("Content-Type", "application/json");
        httpURLConnection.setDoOutput(true);
        ...
        if (responseCode == HttpURLConnection.HTTP_OK) { // success
            ...
            assertEquals(response.toString().contains("\"id\":101010,\"name\":\"Coffee\",\"price\":14.0,\"quantity\":1.0,\"discount\":0.0"),
                    true);
